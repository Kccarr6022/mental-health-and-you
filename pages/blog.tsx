    ...
    return (
        ...
        <div className="w-4/5 mx-auto p-0 sm:p-8 max-w-[1000px]">    
            {initialPosts.map((blogPost, key) => {
                return <>
                <article className="grid grid-cols-1 sm:grid-cols-3 pt-6 pb-4 my-4 bg-secondary-green-transparent" key={key}>
                <div className="flex justify-center col-span-1 px-2">
                    <img src={blogPost.thumbNail} />
                </div>
                <div className="px-2 col-span-1 sm:col-span-2">
                    <h1 className="font-bold text-lg sm:text-xl md:text-2xl lg:text-3xl mb-4">{blogPost.title}</h1>
                    <p className="text-base sm:text-lg md:text-xl lg:text-2xl">{(blogPost.body.length <= 200) ? blogPost.body : blogPost.body.slice(0, 200) + "..."}</p>
                    <div className="text-sm sm:text-base md:text-lg lg:text-xl"><a href={`/blogs/${blogPost.id}`}>continue reading</a></div>
                    <div className="h-6">
                        <span className="flex-container flex-nowrap"><RxCalendar /><time dateTime={blogPost.date.toISOString()}>May 05, 2023</time></span>
                    </div>
                </div>
                </article>
                <hr />
